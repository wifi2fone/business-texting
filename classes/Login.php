...
class Login
{
    ...
    private function getUserData($user_name)
    {
        ...
        if ($this->databaseConnection()) {
            ...
            $query_user = $this->db_connection->prepare('SELECT * FROM directory WHERE username = :user_name');
        ...
        if (isset($_COOKIE['rememberme'])) {
            ...
            if ($hash == hash('sha256', $user_id . ':' . $token . COOKIE_SECRET_KEY) && !empty($token)) {
                ...
                if ($this->databaseConnection()) {
                    ...
                    $sth = $this->db_connection->prepare("SELECT id, username, email FROM directory WHERE id = :user_id
    ...
    private function loginWithPostData($user_name, $user_password, $user_rememberme)
    {
        ...
        // if POST data (from login form) contains non-empty username and non-empty user_password
            ...
            // if user has typed a valid email address, we try to identify him with his email
                ...
                // database query, getting all the info of the selected user
                $query_user = $this->db_connection->prepare('SELECT * FROM directory WHERE email = :user_email');
            ...
            // using PHP 5.5's password_verify() function to check if the provided passwords fits to the hash of that user's password
                ...
                $sth = $this->db_connection->prepare('UPDATE directory '
            ...
            // has the user activated their account with the verification email
                ...
                if (defined('HASH_COST_FACTOR')) {
                    ...
                    if (password_needs_rehash($result_row->user_password_hash, PASSWORD_DEFAULT, array('cost' => HASH_COST_FACTOR))) {
                        ...
                        $query_update = $this->db_connection->prepare('UPDATE directory SET user_password_hash = :user_password_hash WHERE id = :user_id');
    ...
    private function newRememberMeCookie()
    {
        ...
        if ($this->databaseConnection()) {
            ...
            $sth = $this->db_connection->prepare("UPDATE directory SET user_rememberme_token = :user_rememberme_token WHERE id = :user_id");
    ...
    private function deleteRememberMeCookie()
    {
        ...
        if ($this->databaseConnection()) {
            ...
            $sth = $this->db_connection->prepare("UPDATE directory SET user_rememberme_token = NULL WHERE id = :user_id");
    ...
    public function editUserName($user_name)
    {
        ...
        // TODO: maybe this pattern should also be implemented in Registration.php (or other way round)
        } elseif (empty($user_name) || !preg_match('/^[0-9]{2,64}$/i', $user_name)) {
            ...
            if (isset($result_row->id)) {
                ...
                $query_edit_user_name = $this->db_connection->prepare('UPDATE directory SET username = :user_name WHERE id = :user_id');
    ...
    public function editUserEmail($user_email)
    {
        ...
        // user mail cannot be empty and must be in email format
            ...
            if (isset($result_row->id)) {
                ...
                $query_edit_user_email = $this->db_connection->prepare('UPDATE directory SET email = :user_email WHERE id = :user_id');
    ...
    public function setPasswordResetDatabaseTokenAndSendMail($user_name)
    {
        ...
        if (empty($user_name)) {
            ...
            if (isset($result_row->id)) {
                ...
                // database query:
                $query_update = $this->db_connection->prepare('UPDATE directory SET user_password_reset_hash = :user_password_reset_hash,
                ...
                if ($query_update->rowCount() == 1) {
                    ...
                    $this->sendPasswordResetMail($user_name, $result_row->email, $user_password_reset_hash);
                    return true;
                } else {
    ...
    public function editNewPassword($user_name, $user_password_reset_hash, $user_password_new, $user_password_repeat)
    {
        ...
        // if database connection opened
            ...
            // write users new hash into database
            $query_update = $this->db_connection->prepare('UPDATE directory SET user_password_hash = :user_password_hash,
